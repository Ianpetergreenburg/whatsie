class RecipesController < ApplicationController
  ...
  def create
      ...
      if helpers.current_user.recipes.include? @recipe
        @recipe_book = RecipeBook.find_by(recipe_id: @recipe.id, user_id: helpers.current_user_id).destroy
        respond_to do |format|
          format.json { render json: { message: 'Removed!'} }
        end
      else
        RecipeBook.create(recipe_id: @recipe.id, user_id: helpers.current_user_id)
        respond_to do |format|
